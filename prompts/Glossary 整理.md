...
不得僅因英文 term 是地名派生詞、族群名、宗教名或語言名，
就自動在既有中文譯名後加入「人」或「語」。只有來源上下文或既有翻譯決策
明確支持該 sense 時，才可保留或新增；無法確認時保留原條目並列為待確認。
整理或移入 contextual 時，既有 `default` 與 `senses[].zh` 不得自行改寫、音譯或校正，
除非使用者明確提供修正。疑似錯字或不自然譯名只能報告，不得自行替換。
